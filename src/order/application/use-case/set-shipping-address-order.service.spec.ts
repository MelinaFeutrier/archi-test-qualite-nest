...
describe('SetShippingAddressOrderService', () => {
    ...
    beforeEach(() => {
        setShippingAddressOrderService = new SetShippingAddressOrderService(orderRepositoryFake);
    });
    ...
    it('should be able to set the shipping address of an order', async () => {
        // Crée une commande valide (avec moins que le nombre maximum d'articles)
        const order = await new CreateOrderService(orderRepositoryFake).execute({
            ...
            items: [
                ...
                { productName: 'item 5', price: 50, quantity: 1 }, // Respecte le nombre max d'articles
            ],
            shippingAddress: 'Shipping Address',
            invoiceAddress: 'Invoice Address',
        });

        await setShippingAddressOrderService.execute(order.id, 'New Shipping Address');
